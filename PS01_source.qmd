...
-The answer **`r ans2`** appears under the code.
:::
...
-   No, the answer doesn't appear.

-   you can see the answer in the environment stored in the variable a.

-   a shows up in the code chunk and the answer is printed on the line below.
:::
...
-   "^" raises the variable to the number given.
:::
...
-   Sum adds together all the given parameters.
:::
...
-   It removed all the objects from the environment.
:::
...
-   The run button reloaded all the environment objects with their data that we just deleted.
:::
...
-   It adds 2 to each of number in the vector.
:::
...
-   R has chr before the vector and the types are in quotes. 
:::
